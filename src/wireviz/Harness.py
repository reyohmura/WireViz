...
class Harness:
    ...
    def create_graph(self):
        ...
        for _, cable in self.cables.items():
            ...
            attributes = [f'{len(cable.colors)}x' if cable.show_wirecount else '',
                          ...
                          f'{cable.length} mm' if cable.length > 0 else '']
            ...
            for connection in cable.connections:
                if isinstance(connection.via_port, int):  # check if it's an actual wire and not a shield
                    ...
                    if search_color in wv_colors.color_hex:
                        dot.attr('edge', color=f'#000000:{wv_colors.color_hex[search_color]}:#000000', fontname=font)
                    else:  # color name not found
                        dot.attr('edge', color='#000000:#ffffff:#000000', fontname=font)
                else:  # it's a shield connection
                    dot.attr('edge', color='#000000', fontname=font)
    ...
    def output(self, filename, directory='_output', view=False, cleanup=True, fmt='pdf', gen_bom=False):
        ...
        # graph.save(filename=f'{filename}.gv', directory=directory) #NO NEED for graph viz files
        # bom output
        bom_list = self.bom_list()
        #with open(f'{filename}.bom.tsv', 'w',encoding='utf-8_sig') as file:
            #file.write(tuplelist2tsv(bom_list))
        # HTML output
        with open(f'{filename}.html', 'w',encoding='utf-8_sig') as file:
            # file.write('<html><body style="font-family:Arial">')
            file.write('<html><body style="font-family:Arial">')
            cablename=os.path.splitext(os.path.basename(filename))[0]
            file.write('<h1>'+cablename+'</h1>')
            with open(f'{filename}.svg',encoding='utf-8_sig') as svg:
                for svgdata in svg:
                    file.write(svgdata)
